...
let Item = React.createClass({
  ...
  render(){
    ...
    if (this.props.page == 'userInfo') {
      let showValue = '';
      if (this.props.value == null || this.props.value == '未填写') {
        showValue = '未填写';
      } else {
        if (this.props.publicValue) {
          showValue = this.props.value + '(公开)';
        } else {
          showValue = this.props.value + '(不公开)';
        }
      }
      return (
        ...
        <TouchableHighlight activeOpacity={0.8} underlayColor='#18304D' onPress={this.props.func}>
          <View>
            <View style={[styles.listLayout,this.props.top && styles.borderTop]}>
              ...
              <View
                style={{flexDirection:'row',alignItems:'center',flex:1,backgroundColor:'#162a40',justifyContent: 'space-between'}}>
                <Text style={[{fontSize: 15,color: '#ffffff',width:140}]}
                      numberOfLines={1}>{this.props.value ? this.props.value : "未填写"}</Text>
                {this.renderArrow(this.props.showArrow)}
              </View>
            </View>
            <View style={styles.bottomStyle}/>
