...
let MyBizList = React.createClass({
  ...
  _renderRow: function (rowData, sectionID, rowID) {
    return (
      <TouchableHighlight onPress={(rowData.status =!"DRAFT")?() => this.toDetail(MyBizDetail,rowData):null} underlayColor='#000'>
        <View
          ...
          />
          <Text style={{position:"absolute",left:Adjust.width(60),top:0,marginLeft:15, marginTop:15,color:(rowData.status =!"DRAFT")?'white':'#386085'}}>
            {rowData.term + '天'}
          </Text>
          <Text
            style={{position:"absolute",left:Adjust.width(120),top:0, marginLeft:15,marginTop:15,color:(rowData.status =!"DRAFT")?'rgba(175,134,86,1)':'#386085',}}>
            {rowData.amount / 10000 + '万'}
          </Text>
          <Text
            style={{position:"absolute",left:Adjust.width(200),top:0, marginLeft:15, marginTop:15,color:(rowData.status =!"DRAFT")?'white':'#386085'}}
            numberOfLines={1}>
            {rowData.rate + '%'}
          </Text>
